...
namespace UA
{
    ...
    class Program
    {
            ...
            while (true)
            {
                ...
                if (trainingMode)
                {
                    ...
                    if (isKeyToggled(Keys.End))
                    {
                        ...
                        FileInfo[] Files = new DirectoryInfo(Application.StartupPath + @"\darknet\data\img").GetFiles($"{game}*.png"); 
                        string PathOfImg = "";
        ...
        public static void PrepareFiles(string game)
        {
            ...
            {
                DialogResult dialogResult = MessageBox.Show($"Looks like you are missing the mainfiles! Pleas Dm Me on Discord","Mainfiles missing", MessageBoxButtons.OK);
                Process.GetCurrentProcess().Kill();
            }
        }
